...
public class ResultCalculator {
    ...
    private int calculateResultIndex(int matchCount, boolean bonusMatch) {
        ...
        } else if (matchCount == MATCH_FIVE && bonusMatch) {
            return MATCH_FIVE_BONUS; // 5개 + 보너스 일치
        } else if (matchCount == MATCH_FIVE) {
            return MATCH_FIVE; // 5개 일치
        } else if (matchCount == MATCH_FOUR) {
            return MATCH_FOUR; // 4개 일치
        } else if (matchCount == MATCH_THREE) {
            return MATCH_THREE; // 3개 일치
        } else {
            return -1; // 일치하지 않는 경우
        }
    }
