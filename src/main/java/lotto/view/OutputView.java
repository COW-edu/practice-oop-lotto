...
public class OutputView {
    ...
    public void outPutLottoList(int purchasedLottoCounts) {
        System.out.println(String.format(InputMessage.PURCHASED_LOTTO_COUNTS.getMessage(), purchasedLottoCounts));
    ...
    public String outPutLottoTotalResult(int purchasedLottoCounts) {
        ...
        for (int correctNumberCount = RESULT_INDEX_THREE; correctNumberCount <= RESULT_INDEX_SEVEN; correctNumberCount++) {
            ...
            System.out.println(outPutResultComment(resultMessage, winningLottoCountList, correctNumberCount));
            totalWinningMoney += lottoController.calculateTotalWinningMoney(resultMessage, winningLottoCountList, correctNumberCount);
        }
        return String.format("%.1f", totalWinningMoney/(purchasedLottoCounts*CHANGE_PURCHASE_MONEY)*CHANGE_PERCENT);
    }
    private String outPutResultComment(ResultMessage resultMessage, int [] winningLottoCountList, int correctNumberCount) {
    ...
    public void outPutRateOfLotteryWinnings(String totalWinningMoney) {
        System.out.println(String.format(OutputMessage.RATE_OF_LOTTERY_WINNINGS.getMessage(), totalWinningMoney));
    }
}
