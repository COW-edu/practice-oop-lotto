...
public class Application {
    public static void main(String[] args) {
        ...
        try {
            user.buyLotto();
            // LotteryHost 에서 로또 생성 후 User의 LottoTickets 필드에 저장
            user.saveLotto(lh.generateAndSaveLottoTickets(user.getNumberOfLottery()));
            // 로또 매수와 번호 출력
            lh.printLottoNumbersAndQuantity(user.getLottoTickets(), user.getNumberOfLottery());
            // 당첨 번호 입력받기
            ...
            // 당첨 통계 출력
            lh.printResult(user.getNumberOfLottery());
